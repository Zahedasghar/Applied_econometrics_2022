...
$$\hat{Y}_{it}=\alpha_i +\theta_{t}+\beta_3 \, (\text{Treated}_i \times \text{After}_{t})+\epsilon_{it}$$
```{r}
#| fig-height: 6
#| fig-align: center
...
If $\gamma_1$ is statistically significant (i.e. we can reject $H_0$: $\gamma_1 = 0$), we have good evidence that the instrument is relevant.
...
This gives us an unbiased estimator of $\beta_1$, so long as the instrument(s) is valid. 
...
The effect is now a much larger -2.0631 (for every additional 1% of GDP growth per capita from the previous year, the probability of internal conflict decreases by 2.06 (206%). 
