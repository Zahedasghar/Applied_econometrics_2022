...
pc|>count(state)
pc|>count(year)
pc|>summarise(n_distinct(state),n_distinct(year))
...
```{r}
# write your code in this chunk
ggplot(pc)+aes(x=unemployrate,y=appspc)+geom_point()

ggplot(pc)+aes(x=unemployrate,y=appspc)+geom_point()+geom_label(data=pc|>filter(appspc>200),aes(label=state))

pc1<-pc|>filter(appspc<200)
  ggplot(pc1)+aes(x=unemployrate,y=appspc)+geom_point()
...
```{r}
# write your code in this chunk
POL<-lm(appspc~unemployrate,data=pc)
POL_nooutlier<-lm(appspc~unemployrate,data=pc1)
tidy(POL)
tidy(POL_nooutlier)
```
...
```{r}
# write your code in this chunk
fe<-lm(appspc~unemployrate+factor(state),data=pc)
tidy(fe)
```
...
```{r}
# write your code in this chunk
demeaned<-feols(appspc~unemployrate|factor(state),data=pc)
tidy(demeaned)
```
